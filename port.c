...
static int32_t s_io0_trigger_pin;
...
void loader_port_delay_ms(uint32_t ms) { usleep(ms * 1000); }

void loader_port_start_timer(uint32_t ms) { s_time_end = clock() + (ms * (CLOCKS_PER_SEC / 1000)); }
...
void loader_port_debug_print(const char* str) { printf("DEBUG: %s\n", str); }
